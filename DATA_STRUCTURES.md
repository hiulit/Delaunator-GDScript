...
After constructing a `var delaunay = Delaunator.new(points)` object, it will have a triangles array and a halfedges array, both indexed by half-edge id. What's a half-edge?
...
We can also use the half-edges of a triangle to find the adjacent triangles. Each half-edge's opposite will be in an adjacent triangle, and the `triangle_id_to_edge_id(t)` helper function will tell us which triangle a half-edge is in:
...
```gdscript
func triangle_center(points, delaunay, t, c = "circumcenter"):
...
Note that this requires any incoming half-edge that leads to the point. If you need a quick way to find such a half-edge given a point, it can be useful to build an index of these half-edges. For an example, see the modified version of `draw_each_voronoi_cell` at the end of the page.
